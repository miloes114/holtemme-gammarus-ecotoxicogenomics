...
- S1 and S2 retain the exact V9.1 SI publication rasters from the previous v1.0.0 release. S1 is an upstream chemistry result with graphical-only legend repair; S2 is the exact three-panel SI composite backed by frozen DP01 source tables. No upstream chemistry mapping is reconstructed.
...
## P0 manuscript/SI correction — resolved in v1.0.1

The v1.0.1 SI candidate removes the obsolete caption wording and carries the final six-author metadata and 16-gene S5 panel. The previous caption was an obsolete, blocking release note; that issue is resolved in this candidate. The authoritative primary chemistry source remains:
...
The v1.0.1 Zenodo candidate contains the broader DP01-DP07 payload, package checksums, and the 17 publication-artifact source entries recorded in `config/zenodo_deposit_manifest.tsv`: [10.5281/zenodo.22915067](https://doi.org/10.5281/zenodo.22915067).
